...
describe('Promise',()=>{
    ...
    it('2.2.2如果onFulfilled是函数', done=>{
        ...
        const promise = new Promise(resolve=>{
            ...
            resolve(2333)
            setTimeout(()=>{
                assert(promise.state==='fulfilled', 'fulfilled')
                assert.isTrue(succeed.called)
                assert(succeed.calledWith(233),'233')
                done()
            },0)
        })
    ...
    it("2.2.3 如果onRejected是函数", done => {
        ...
        const promise = new Promise((resolve, reject) => {
            ...
            reject(2333);
            setTimeout(() => {
                assert(promise.state === "rejected", 'rejected');
                assert.isTrue(fail.calledOnce);
                assert(fail.calledWith(233), '233');
                done();
            }, 0);
        });
        promise.then(null, fail);
